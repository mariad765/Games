...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-4, 0.75f, 0));
...

    {
        ...
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(4, 0.85f, 0.7f));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(100.0f), glm::vec3(1, 0, 0));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
...
{
    float step = 0.5f; // pasul de modificare
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_1)) {
            angular_step = angular_step + deltaTime * cameraSpeed;
            if (mode == "perspective") {
                projectionMatrix = glm::perspective(RADIANS(angular_step), window->props.aspectRatio, zNear, zFar);
            }
        }
        ...
        // Crește lățimea (stânga și dreapta)
        if (window->KeyHold(GLFW_KEY_7)) {
            top += step * deltaTime;
            bottom -= step * deltaTime;
            if (mode == "ortographic") {
                projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
            }
        }
        // Scade lățimea (stânga și dreapta)
        ...
        // Crește înălțimea (sus și jos)
        if (window->KeyHold(GLFW_KEY_9)) {
            right += step * deltaTime;
            left -= step * deltaTime;
            if (mode == "ortographic") {
                projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
            }
        }
        // Scade înălțimea (sus și jos)
...
void Lab5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->GetSpecialKeyState() == 0) {
            ...
            camera->RotateFirstPerson_OX((-1) * sensivityOY * deltaY);
            camera->RotateFirstPerson_OY((-1) * sensivityOX * deltaX);
        ...
        if (window->GetSpecialKeyState() & GLFW_MOD_CONTROL) {
            ...
            // OX and OY using `deltaX` and `deltaY`. Use the sensitivity
            // variables for setting up the rotation speed.
            camera->RotateThirdPerson_OX((-1) * sensivityOY * deltaY);
            camera->RotateThirdPerson_OY((-1) * sensivityOX * deltaX);
