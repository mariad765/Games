...
void Tema1_Vis2D::Init()
{
    ...
    Mesh* square1 = object2D::CreateSquare1("square1", corner, length, glm::vec3(1, 0, 0));
...
void Tema1_Vis2D::Update(float deltaTimeSeconds)
{
    ...
    // Actualiza?i logica ferestrei cu pozi?ia ?i zoom-ul camerei
    logicSpace.x = cameraPosX;
    logicSpace.y = cameraPosY;
    logicSpace.width = 4 * cameraZoom;   // Ajusta?i l??imea în func?ie de zoom
    logicSpace.height = 4 * cameraZoom;  // Ajusta?i în?l?imea în func?ie de zoom
...
void Tema1_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    // Verific? dac? se apas? tastele W, A, S, D pentru transla?ie
    if (window->KeyHold(GLFW_KEY_W)) {
        cameraPosY += 2.0f * deltaTime; // Muta?i camera în sus
    }
    if (window->KeyHold(GLFW_KEY_S)) {
        cameraPosY -= 2.0f * deltaTime; // Muta?i camera în jos
    }
    if (window->KeyHold(GLFW_KEY_A)) {
        cameraPosX -= 2.0f * deltaTime; // Muta?i camera la stânga
    }
    if (window->KeyHold(GLFW_KEY_D)) {
        cameraPosX += 2.0f * deltaTime; // Muta?i camera la dreapta
    }

    // Verific? dac? se apas? tastele Z ?i X pentru zoom
    if (window->KeyHold(GLFW_KEY_Z)) {
        cameraZoom += 0.1f * deltaTime; // Zoom in
    }
    if (window->KeyHold(GLFW_KEY_X)) {
        cameraZoom -= 0.1f * deltaTime; // Zoom out
        if (cameraZoom < 0.1f) {
            cameraZoom = 0.1f; // Limiteaz? zoom-ul
        }
    }
