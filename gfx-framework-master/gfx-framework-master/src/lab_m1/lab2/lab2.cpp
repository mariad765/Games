...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices
        {
            ...
            // TODO(student): Complete the vertices data for the cube mesh
             //one face
            /* VertexFormat(glm::vec3(-1, -1,  1), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
             VertexFormat(glm::vec3(0, 0,  1), glm::vec3(1, 0, 0), glm::vec3(0.2, 0.8, 0.6)),
             ...
             VertexFormat(glm::vec3(1, 1, 0), glm::vec3(1, 0, 0), glm::vec3(0.2, 0.8, 0.6)),d
    ...
    vector<VertexFormat> tetraVertices
    {
        VertexFormat(glm::vec3(-0.5f, 0, 0), glm::vec3(1, 0, 0), glm::vec3(1, 0, 0)), // A
        VertexFormat(glm::vec3(0.5f, 0, 0), glm::vec3(0, 1, 0), glm::vec3(0, 1, 0)), // B
        VertexFormat(glm::vec3(0, 0, 1), glm::vec3(0, 0, 1), glm::vec3(0, 0, 1)), // C
        VertexFormat(glm::vec3(0, 0.5f, 0.5f), glm::vec3(1, 0, 0), glm::vec3(0, 0, 1)), // D (top)
    };

    vector<unsigned int> tetraIndices =
    {
        0, 1, 2, // Base (clockwise)
        3, 0, 1, // Side A (clockwise)
        3, 1, 2, // Side B (clockwise)
        3, 2, 0  // Side C (clockwise)
    };

    meshes["tetrahedron"] = new Mesh("generated tetrahedron");
    meshes["tetrahedron"]->InitFromData(tetraVertices, tetraIndices);

    // TODO(student): Create a square using two triangles with
    // opposing vertex orientations.
    vector<VertexFormat> squareVertices
    {
        VertexFormat(glm::vec3(-0.5f, -0.5f, 0), glm::vec3(0, 1, 0), glm::vec3(1, 1, 0)), // Bottom-left
        VertexFormat(glm::vec3(0.5f, -0.5f, 0), glm::vec3(0, 1, 0), glm::vec3(1, 1, 0)), // Bottom-right
        VertexFormat(glm::vec3(0.5f, 0.5f, 0), glm::vec3(0, 0, 1), glm::vec3(1, 1, 0)),  // Top-right
        VertexFormat(glm::vec3(-0.5f, 0.5f, 0), glm::vec3(0, 0, 1), glm::vec3(1, 1, 0)),  // Top-left
    };

    // Indices for the two triangles
    vector<unsigned int> triangle1Indices = { 0, 1, 2 }; // First triangle
    vector<unsigned int> triangle2Indices = { 0, 2, 3 }; // Second triangle

    meshes["triangle1"] = new Mesh("generated triangle 1");
    meshes["triangle1"]->InitFromData(squareVertices, triangle1Indices);

    meshes["triangle2"] = new Mesh("generated triangle 2");
    meshes["triangle2"]->InitFromData(squareVertices, triangle2Indices);
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["triangle1"], shaders["VertexColor"], glm::vec3(0, -0.5f, 0), glm::vec3(0.5f));

    // Render the second triangle
    glCullFace(GL_FRONT); // Culling front faces
    RenderMesh(meshes["triangle2"], shaders["VertexColor"], glm::vec3(0, -0.5f, 0), glm::vec3(0.5f));
   ...
   // RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(0, -0.5f, 0), glm::vec3(0.5f));


    // TODO(student): Draw the mesh that was created with `CreateMesh()`
    glCullFace(cullFace);
    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(1.5f, 0.5f, 0), glm::vec3(0.5f));
...
void Lab2::OnKeyPress(int key, int mods)
{
    ...
    // Toggle between GL_BACK and GL_FRONT culling with G key
    if (key == GLFW_KEY_G)
    {
        cullFace = (cullFace == GL_BACK) ? GL_FRONT : GL_BACK;
        std::cout << "Culling mode switched to: " << (cullFace == GL_BACK ? "GL_BACK" : "GL_FRONT") << std::endl;
    }
